...
# loading data
dat0 <- read.table(file="counts_R11.txt", header = T, sep = "\t")
dat1 <- dat0[,2:17]
...
flt1 <- which(rowSums(dat1<10)>10)
flt2 <- which(rowSums(dat1)<220)
...
desi <- cbind(c(rep("VAD",9),rep("VAS",7)),
              c(rep("N_inf",3),rep("inf",6),rep("N_inf",3),rep("inf",4)),
              c("r1","r2","r3","r1","r2","r3","r4","r5","r6","r1","r2","r3","r1","r2","r3","r4"))
...
res_infectioneffectforVAD <- results(dds2, contrast=c("is.inf","inf","N_inf"),alpha=0.05)
...
write.csv(counts(dds2, normalized=TRUE), file = "20181119_R11colon_highcount_normalizedbyDESeq2.csv",quote = F)
## print an excel of all p and adj P values of all genes
write.csv(res_infectioneffectforVAD, file = "20181119_R11colon_results_infectioneffect_forVAD.csv",quote = F, row.names = rownames(dat1))
...
res_VA_all <- res_VA
res_VA_all$ENTREZID <- mapIds(org.Mm.eg.db,
...
res_VA_all$Fullname <- mapIds(org.Mm.eg.db,
                           keys=row.names(res_VA_all),
                           keytype="SYMBOL",
                           column="GENENAME")


#dim(res_outliers_excluded)
dim(res_VA_all)
...
####print normalized count, relatively high expression count, for all genes
write.csv(counts(dds2, normalized=TRUE), file = "20181206_R11colon_highcount_normalizedbyDESeq2.csv",quote = F)
## print an excel of all p and adj P values of all genes
write.csv(res_VA_all, file = "20181206_colon_results_VAeffectWithoutinfection.csv",quote = F, row.names = rownames(dat1))
...
# adj p of 2082 genes ="NA," including Cyp26b1 and Ccr9, but mean values are low, 
#needs to remove those
dim(allgene)

allgene<- na.omit(allgene)
#将adj p一列显示NA的gene行全部移除
dim(allgene)
...
write.csv(allgene,file="20181206allgene.csv",quote=F)
...
write.csv(gene_up,file="20181206gene_up.csv",quote=F)
#将筛选的上调基因检测报告，保存为csv格式
write.csv(gene_down,file="20181206gene_down.csv",quote=F)
#将筛选的下调基因检测报告，保存为csv格式
write.csv(gene_dif,file="20181206gene_dif.csv",quote=F)
...
gene_dif_exp_VA<-counts(dds2, normalized=TRUE)[rownames(gene_dif),-c(4,5,6,7,8,9,13,14,15,16)]
#提取差异基因的基因表达值
head(gene_dif_exp_VA)
#rownames(gene_dif_exp)<-gene_dif$symbols
#将差异基因表达矩阵的行名转化为gene symbol ID名

write.csv(gene_dif_exp_VA,file="gene_dif_exp_VA.csv",quote=F)
#将筛选的差异基因表达，保存为csv格式

tiff(file="gene_dif_pheatmap_VAeffectwoInfection.tif",res=300,units='in',width=30,height=30)
#打开tiff绘图设备

pheatmap(gene_dif_exp_VA,cluster_cols = FALSE,cluster_rows = TRUE,
         color=colorRampPalette(c("blue","white","red"))(100),fontsize_row=26,fontsize_col=26,scale="row",border_color=NA)
#画差异基因热图
...
# load data
allgene2 <- read.csv(file="20181206allgene.csv", header=T,row.names=1)
...
tiff(file="20181223volcano.tif",res=100,units='in',width=10,height=10)
...
xlab("Log2 Fold Change")+ylab("-Log10(padj)")+
  ggtitle("")+theme(plot.title=element_text(hjust=0.5,size=24))+
  geom_point()+
...
tiff(file="20181223volcano_label.tif",res=100,units='in',width=10,height=10)
...
p=ggplot(results2,aes(x=log2FoldChange,y=-log10(padj)),colour=threshold)+
  xlab("Log2 Fold Change")+ylab("-Log10(padj)")+ggtitle("")+
...
tiff(file="20181223volcano_size.tif",res=100,units='in',width=10,height=10)
####aim: make size of point proportional to Mean expression strength (baseMean)
ggplot(allgene2,aes(x=log2FoldChange,y=-log10(padj),colour=threshold))+xlab("Log2 Fold Change")+ylab("-Log10(padj)")+ggtitle("")+theme(plot.title=element_text(hjust=0.5))+
geom_point(size=allgene2$baseMean*0.0001)+geom_vline(xintercept = c(-1,1))+geom_hline(yintercept = -log10(0.05))+theme(panel.grid =element_blank())+scale_colour_manual(values=c("blue", "transparent", "red"))+expand_limits(x=c(-8,8))+
theme(axis.text.x = element_text(colour="grey20",size=20,angle=0,hjust=.5,vjust=.5,face="plain"),
        axis.text.y = element_text(colour="grey20",size=20,angle=0,hjust=1,vjust=0,face="plain"),  
        axis.title.x = element_text(colour="grey20",size=24,angle=0,hjust=.5,vjust=0,face="plain"),
        axis.title.y = element_text(colour="grey20",size=24,angle=90,hjust=.5,vjust=.5,face="plain"))
...
res_VA_enrich<-allgene
res_VA_enrich$entrez <- mapIds(org.Mm.eg.db,
                         keys=row.names(allgene),
                         keytype="SYMBOL",
                         column="ENTREZID",
                         multiVals="first")


allgene3<- na.omit(res_VA_enrich)
#将entrez一列显示NA的gene行全部移除
dim(allgene3)

write.csv(allgene3,file="20181208allgene3.csv",quote=F)
#将新的所有基因的检测报告，输出为csv格式。
...
gene_dif<-allgene3[allgene3$type!="normal",]
#筛选非差异基因，并存储于变量gene_dif中

write.csv(gene_up,file="20181208gene_up.csv",quote=F)
#将筛选的上调基因检测报告，保存为csv格式
write.csv(gene_down,file="20181208gene_down.csv",quote=F)
#将筛选的下调基因检测报告，保存为csv格式
write.csv(gene_dif,file="20181208gene_dif.csv",quote=F)
#将筛选的差异基因检测报告，保存为csv格式
