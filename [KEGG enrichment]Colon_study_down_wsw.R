...
down_genelist<-gene_down$log2FoldChange
#提取差异基因的变化倍率值，赋值给dif_genelist
names(down_genelist)<-gene_down$entrez
#用EntrezID对差异基因的变化倍率进行命名

down_kk <- enrichKEGG(gene=EG_down,universe= EGU,organism= 'mmu',pvalueCutoff = 0.05)
...
#将差异基因在KEGG进行富集分析。
write.table(down_kk,file="down_kk.txt",sep="\t",quote=F,row.names=F)
#将差异基因在在KEGG富集分析的结果表进行存储。
barplot(down_kk,showCategory=10)
#KEGG富集条形图
dotplot(down_kk,showCategory=10)
#KEGG富集散点图
enrichMap(down_kk)
#富集通路之间的关系图
cnetplot(down_kk,showCategory =10,categorySize="pvalue", foldChange=genelist,vertex.label.cex=1)
#富集通路与基因之间的关系图



browseKEGG(down_kk, 'mmu05150')
#打开通路图的网页。
...
#biocLite("pathview")
library(pathview)
mmu05150 <- pathview(gene.data=down_genelist,pathway.id = "mmu05150",species= "mmu",limit=list(gene=max(abs(genelist)), cpd=1))
#做可视化的通路图